...
?>
 ...
 <h1 style="margin-bottom: 3%; margin-top: 3%; text-align: center;">Regiões Cadastrados</h1>
 ...
 <form action="../Services/validar_cadastro_JOIN.php" method="post">
        ...
        <div class="row" style="margin-bottom: 3%;">
            <div class="col-md-4">
                ...
                <select class="form-select" name="nome_pais">
                        <option selected value="" selected>Selecione o País</option>
                        <?php while($opcoes_pais = $resultado_pais->fetch(PDO::FETCH_ASSOC)) :  ?>
                        <option value="<?= htmlspecialchars($opcoes_pais['id']) ?>" ><?= htmlspecialchars($opcoes_pais['nome_pais']) ?></option>
                        <?php endwhile ?>
                        </select>
            ...
            <div class="col-md-4">
                ...
                <select class="form-select" name="nome_estado">
                        <option selected value="" selected>Selecione o Estado</option>
                        <?php while($opcoes_estado = $resultado_estado->fetch(PDO::FETCH_ASSOC)) :  ?>
                         <option value="<?= htmlspecialchars($opcoes_estado['id']) ?>" ><?= htmlspecialchars($opcoes_estado['nome_estado']) ?></option>
                        <?php endwhile ?>
                        </select>
            ...
            <div class="col-md-4">
                ...
                <select class="form-select" name="nome_cidade">
                        <option selected value="" selected>Selecione a Cidade</option>
                        <?php while($opcoes_cidade = $resultado_cidade->fetch(PDO::FETCH_ASSOC)) :  ?>
                        <option value="<?= htmlspecialchars($opcoes_cidade['id']) ?>" ><?= htmlspecialchars($opcoes_cidade['nome_cidade']) ?></option>
                        <?php endwhile ?>
                        </select>
        ...
        <div class="row">
            <div class="col-md-4">
             <button type="submit" name="enviar" class="btn btn-primary">Cadastrar país</button>
            </div>
  ...
  <tbody>
    ...
    <tr>
        ...
        <td> 
            ...
            <a class="btn btn-danger" href="../Services/deletar_JOIN.php?id=<?= $row['id']; ?>" onclick="return confirm('Voce tem certeza que quer deletar?')">Deletar</a></td>   
    </tr>
    <?php endwhile; ?>
  </tbody>
</table>
